...
prob_class0 = C/row;								% find probability of class
sigma = cov(train_data(:,1:9));					% calculates covariance of whole training dataset
...
for i=1:row										% test on number of sample points of test data (D1)
	...
	pred_Y = -1;								% variable to store predicted value
	Delta0 = X* inv(sigma)* mean_class0'-0.5* mean_class0 *inv(sigma)* mean_class0'+log(prob_class0);	% probability of class0 (LDA formula)
	Delta1 = X* inv(sigma)* mean_class1'-0.5* mean_class1 *inv(sigma)* mean_class1'+log(prob_class1);	% probability of class1 (LDA formula)
	if Delta0 > Delta1							% if Delta0 is greater classify as class 0 else class 1
		pred_Y = 0;
...
for i=1:row
	...
	pred_Y = -1;
	Delta0 = X* inv(sigma)* mean_class0'-0.5* mean_class0 *inv(sigma)* mean_class0'+log(prob_class0);
	Delta1 = X* inv(sigma)* mean_class1'-0.5* mean_class1 *inv(sigma)* mean_class1'+log(prob_class1);
	if Delta0 > Delta1
		pred_Y = 0;
	elseif Delta0 < Delta1
		pred_Y = 1;
...
[C,D] = size(class0);	
prob_class0 = C/row;
sigma = cov(train_data(:,1:9));						% calculates covariance of whole dataset
mean_class1 = mean(class1);							% calculates mean for each feature for class = 1
mean_class0 = mean(class0);							% calculates mean for each feature for class = 0
...
LDA_avg_error_rate_10Folds = (error_fold1+error_fold2+error_fold3+error_fold4+error_fold5+error_fold6+error_fold7+error_fold8+error_fold9+error_fold10)/10
% LDA_avg_error_10Fold = 25.5134%